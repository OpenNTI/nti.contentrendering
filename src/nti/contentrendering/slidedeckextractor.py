...
def process_ntislidedecks(topic, result, containers):
    ...
    for node in topic.dom('object').filter('.ntislidedeck'):
        d = {	
				 'class': 'ntislidedeck',
...
def process_book(book, containers):
    ...
    def _loop(topic):
        ...
        process_elements(topic, slides,
                         'application/vnd.nextthought.slide', containers)
        process_elements(topic, slides,
                         'application/vnd.nextthought.ntislide', containers)
        process_elements(topic, videos,
                         'application/vnd.nextthought.ntislidevideo', containers)
...
def save_index_file(outpath, data, dom_ntiid):
    ...
    for topic_ntiid, iterable in data.items():
        ...
        for item in iterable:
            ntiid = item.get('ntiid') or item.get('slidedeckid')
            if not ntiid:
                continue
            items[ntiid] = item
