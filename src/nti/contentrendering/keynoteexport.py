...
etree_QName = getattr(etree, 'QName')
...
def _sel(s): 
	return CSSSelector(s, namespaces=_NAMESPACES)
...
def _localname(element):
	return etree_QName(element).localname if element is not None else None
	...
	def __str__(self):
		...
		for kid in self.children:
			...
			if not isinstance(kid, _List) and kid != _FORCED_NEWLINE:
				line = line + '\item '
			line = line + unicode(ILatexContentFragment(kid))
			lines.append(line)
...
class _Slide(object):
	...
	def __str__(self):
		...
		lines = [u'\section{' + ILatexContentFragment(self.text_elements[0]) + '}'
				 if self.text_elements
				 else '']

		for kid in self.text_elements:
			lines.append(ILatexContentFragment(kid))
		return '\n'.join((unicode(line) for line in lines))
...
class _Presentation(object):
	...
	def to_latex(self):
		...
		for slide in self.slides:
			lines.append(ILatexContentFragment(slide))
	...
	def _make_fragments(p):
		...
		try:
			if p.find('.//{' + _NAMESPACES['sf'] + '}br') is not None:
				fragments.append(LatexContentFragment('\\\\\n'))
		except IndexError:
			pass
		return fragments
	...
	for slide in SLIDE(keynote):
		...
		for text_or_img in TEXT_OR_IMAGE_DATA(slide):
			...
			if _localname(text_or_img) == 'text':
				text = text_or_img
				for p in P(text):
					text_body = etree_tostring(p, encoding=unicode)
...
if __name__ == '__main__':
	...
	doc = etree_parse(filename)
	pres = extract_text(doc, jobname, 
						os.path.join(os.path.dirname(filename), 'SlideImages'))
	print(pres.to_latex())
