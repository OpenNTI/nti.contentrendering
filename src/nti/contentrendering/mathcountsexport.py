...
class _ElementPlainTextContentFragment(PlainTextContentFragment):
	...
	def __init__(self, element=None):
		# XXX: __new__ does all the actual work, because these are immutable as strings
...
def _p_to_content(p, include_tail=True):
	...
	return LatexContentFragment(' '.join([ILatexContentFragment(x) for x in accum]))
	...
	for story in stories:
		...
		for problem in (pyquery.PyQuery(story))(b"p.problems"):
			...
			if _t is not None:
				...
				worksheet.problems[len(worksheet.problems) - 1].question = ' '.join(
							[worksheet.problems[len(worksheet.problems) - 1].question, accum])
