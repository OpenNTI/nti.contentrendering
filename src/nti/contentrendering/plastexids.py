...
def _make_ntiid(document, local, local_prefix='', nttype='HTML'):
    ...
    try:
        base = document.userdata['specific_ntiid']
    except KeyError:
        base = None
...
def _section_ntiid(self):
    ...
    if local is None:
        # not idempotent, has side effects, call only once
        local = _preferred_local_part(self)
    if not local:
...
class NTIIDMixin(object):
    ...
    filenameoverride = property(_section_ntiid_filename,
                                _set_section_ntiid_filename)
...
def _par_id_get(self):
    ...
    if      first_non_blank_child is not None \
        and first_non_blank_child.nodeName == 'label' \
        and 'label' in first_non_blank_child.attributes:
