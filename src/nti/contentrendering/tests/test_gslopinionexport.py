...
class TestGSL(ContentrenderingLayerTest):
    ...
    def test_runthrough(self):
        ...
        assert_that(out.getvalue(),
                    contains_string(br'\href{/scholar\_case'))
        assert_that(out.getvalue(),
                    contains_string(br'\subsection{Opinion of the Court'))
        assert_that(out.getvalue(),
                    contains_string(br'\section{MR. JUSTICE BLACKMUN'))
        assert_that(out.getvalue(),
                    contains_string(br'\section{MR. JUSTICE WHITE'))
        assert_that(out.getvalue(),
                    contains_string(br'{ \textit{Bushman,} 1 Cal. 3d, at 773, 463 P. 2d, at 730, }'))
        assert_that(out.getvalue(),
                    contains_string(br'created. \footnote{It is illuminating'))
        assert_that(out.getvalue(),
                    contains_string(br'\textbf{403 U.S. 15 (1971)}'))

    @fudge.patch('requests.get', 'nti.contentrendering.gslopinionexport.sys')
    def test_main(self, fake_get, fake_sys):
        path = os.path.join(os.path.dirname(__file__), 'gslopinion.html')
        with open(path, 'rU') as fp:
            data = fp.read()
        fake_get.expects_call().returns_fake().has_attr(text=data)
        fake_sys.has_attr(argv=['a', 'b'], 
                          stdout=fudge.Fake(name='stdout').provides('write'))
        contentLocation = 'COHEN_v__CALIFORNIA_'
        try:
            os.chdir('/tmp')
            # as we've already loaded it
            gslopinionexport.main(_do_config=False)
