...
class _TreePlainTextContentFragment(PlainTextContentFragment):
...
class _Container(LatexContentFragment):
	__slots__ = LatexContentFragment.__slots__ + ('children', '__parent__')
	children = ()
	__parent__ = None
...
def _p_to_content(footnotes, p, include_tail=True):
	...
	return _Container(LatexContentFragment(' '.join([ILatexContentFragment(x) for x in accum])))
	...
	for i in footnotes:
		...
		if anchor.get('name') == ref_to \
		  or  _text_of(i).startswith(ref_to):
			...
			accum.append(_p_to_content(footnotes, i))
	return  _Footnote(' '.join([ILatexContentFragment(x) for x in accum]))
...
def _opinion_to_tex(doc, output=None, base_url=None):
	...
	def _print(node):
		print(ILatexContentFragment(node).encode('utf-8'), file=output)
		print(file=output)
		for child in getattr(node, 'children', ()):
			_print(ILatexContentFragment(child))
