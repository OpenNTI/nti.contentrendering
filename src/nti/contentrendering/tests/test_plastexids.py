...
class TestPlastexIds(unittest.TestCase):
    ...
    def test_cross_doc_refs(self):
        ...
        assert_that(context.labels, has_entry(
            'A', has_property('ntiid', chapter1_ntiid)))
        assert_that(context.labels, has_entry(
            'B', has_property('ntiid', chapter2_ntiid)))
...
class TestPlastexIdsRender(ContentrenderingLayerTest):
    ...
    def test_cross_doc_ref_links(self):
        ...
        with RenderContext(simpleLatexDocumentText(preludes=(br'\usepackage{nti.contentrendering.plastexpackages.ntilatexmacros}',
            ...
            eqn_ntiid = source_context.dom.getElementsByTagName(
                'equation')[0].ntiid
            fig_ntiid = source_context.dom.getElementsByTagName(
                'figure')[0].getElementsByTagName('caption')[0].ntiid
            assert_that(eqn_ntiid, is_(
                'tag:nextthought.com,2011-10:testing-HTMLequation-temp.equation.equation'))
            assert_that(fig_ntiid, is_(
                'tag:nextthought.com,2011-10:testing-HTMLfigure-temp.figure.figure'))
